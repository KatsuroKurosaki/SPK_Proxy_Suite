	...
	public void onRegisterEvent(RegisterCustomEvent e) {
		if (e.getSuccess()) {
			...
			if (ProxyServer.getInstance().getServers()
					.containsKey(this.plugin.getMainCnf().getYaml().getString(Global.CONFNODE_LOBBY_SERVER))) {
				e.getPlayer().connect(ProxyServer.getInstance()
						.getServerInfo(this.plugin.getMainCnf().getYaml().getString(Global.CONFNODE_LOBBY_SERVER)));
			} else {
